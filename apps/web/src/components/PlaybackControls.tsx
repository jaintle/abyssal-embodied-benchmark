...
/**
 * PlaybackControls — 2D overlay controls for replay playback (Phase 4.5)
 *
 * Contains: scrubber, play/pause, restart, speed selector, camera toggle.
 * The scrubber is an HTML range input; seeking pauses playback and snaps
 * the agent to the selected step via the onSeek callback.
 */
  ...
  return (
    <div style={CONTAINER}>
      ...
      <div style={CONTROLS_ROW}>
        ...
        {/* Camera mode toggle */}
        <span style={LABEL}>CAM</span>
        <button
          style={cameraMode === "overview" ? BTN_BASE : BTN_ACTIVE}
          onClick={onCameraToggle}
          title={cameraMode === "overview" ? "Switch to follow camera" : "Switch to overview camera"}
        >
          {cameraMode === "overview" ? "OVR" : "FOL"}
        </button>
      </div>
    </div>
  );
